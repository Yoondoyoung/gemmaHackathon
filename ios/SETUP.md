...
- [ ] 기본 **Mesh OFF** (재구성·스캔 중지, 부하↓). 필요할 때만 우상단 **Mesh ON**
      → sceneReconstruction 켜짐 + 앵커 스캔. OFF 시 메쉬 앵커 제거
- [ ] Mesh ON 시 분류 컬러 오버레이 (floor=녹, wall=주황, door=파랑, window=청록)
- [ ] Mesh ON 후 상태줄에 `mesh: floor center 3.2m · door …` 형태가 수 초 내 등장
- [ ] 열린 복도에서 `"Path clear ahead, about N meters"` (12초 쿨다운)
- [ ] 갈림길: 전방 바닥+측면 바닥이 ~0.75초 유지될 때만
      `"Opening on your left/right"` / `"Fork ahead…"` (넓은 홀 오탐 억제)
- [ ] 같은 갈림길을 다시 지나가면 `"You are passing a turn…"` (ARKit 월드 좌표, GPS 아님)
- [ ] backpack 등을 한번 비춘 뒤 `"Where's my backpack?"` →
      `"Your backpack is about N meters behind you on your left…"` (YOLO+LiDAR+AR 포즈)
- [ ] 표지판/물체 지나친 뒤 `"Did I pass the restroom?"` → Gemma 없이 Yes/No
      (OCR이 WC/TOILET이어도 restroom 동의어로 매칭)
- [ ] WorldMap **자동저장** (tracking OK + 메쉬 있을 때 ~20초 / 백그라운드) + 재실행 시 자동 로드. 수동 Save/Load도 동일 파일
      (인터넷 불필요. 같은 공간을 다시 비추면 tracking OK)
- [ ] 문/창에 다가가면 `"door ahead, N meters"` / `"window ahead, N meters"`
- [ ] 벽/문/창은 화면 중앙 1/3만, 옆벽 음성 경고 거의 없음
